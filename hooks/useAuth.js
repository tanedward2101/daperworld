    ...
    if (connector) {
      activate(connector, async (error) => {
        if (error instanceof UnsupportedChainIdError) {
          // for bridge page, we need to support both ETH and BSC mainnet
          // so this should be handled using a new useEffect
          addNotification({
            title: 'Chain Error',
            message: 'Please check if Ethereum or BSC main network is chosen.',
            type: 'danger',
          });
          activate(connector);
        } else {
